...
fn benchmark(io: std.Io, num_seeds: u32) !void {
    ...
    const time_start = std.Io.Clock.now(std.Io.Clock.real, io);
    ...
    const time_end = std.Io.Clock.now(std.Io.Clock.real, io);
...
/// Solve a single board and print the solution path.
fn solveBoard(input_board: Board) !void {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
...
pub fn main(init: std.process.Init) !void {
    //return benchmark(init.io, 30);
    _ = init;
    return solveBoard(game.createRandomBoardFreecellSolitaireDotCom(64368));
}
